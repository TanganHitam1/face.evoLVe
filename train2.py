...
from util.utils import get_val_data, make_weights_for_balanced_classes, separate_irse_bn_paras, separate_resnet_bn_paras, warm_up_lr, schedule_lr, perform_val, get_time, buffer_val, AverageMeter, accuracy
...
if __name__ == '__main__':
    ...
    tqdm_epoch = tqdm(range(NUM_EPOCH), desc = "Epoch", leave=False, total=NUM_EPOCH)

    scaler = GradScaler()

    path = f"{MODEL_ROOT}/metric{HEAD_NAME}_loss{LOSS_NAME}_lr{LR}_batch{BATCH_SIZE}_epoch{NUM_EPOCH}_opt{OPTIMIZER.__class__.__name__}"
    ...
    for epoch in tqdm_epoch: # start training process
        ...
        for inputs, labels in tqdm_train: # start to iterate over data_loader (batch training steps
            ...
            tqdm_train.set_description("Epoch: {}/{} Loss: {:.4f} Prec@1: {:.3f} Prec@5: {:.3f}".format(epoch + 1, NUM_EPOCH, losses_train.avg, top1_train.avg, top5_train.avg))
        ...
        print("Epoch {}/{}, Evaluation: LFW Acc: {}, Theshold: {}".format(epoch + 1, NUM_EPOCH, accuracy_lfw, best_threshold_lfw))
